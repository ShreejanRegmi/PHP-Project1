...
session_start();//session is started for each user
	...
	if(isset($_GET['id'])){//if id variable is set in url
		$selectprpstmt=$pdo->prepare("DELETE FROM tbl_products WHERE product_id=:id");//delete query to delete product
		if($selectprpstmt->execute($_GET)){//if query is executed successfully
			header('Location:listProduct.php?msg= Deletion successful');//redirected to product list with message
		}
		else{
			$_GET['msg']='Deletion of product unsuccessful'; //if not, error message is shown
		}
	}
	...
	if(!isset($_GET['id'])){//if not id is set for deletion of product
		echo '<h5> No product selected for deletion </h5>';//error message is shown
	}
